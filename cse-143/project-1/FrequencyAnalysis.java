// Adam Blank
// 1/7/2015
// The frequency analysis class attempts to solve transposition ciphers 
...
public class FrequencyAnalysis {
   /* Characters ordered from most to least frequently used */
    ...
    /* The threshold used to determine if the percentages are far enough apart to
    ...
    /* post: Attempts to use the most definitive frequencies to fill in letters 
             in the cryptogram. This function will overwrite any previous decoding 
             work that was done.  */
    public void decipherEasyLetters() {
        /* Begin with the cipherText we were given.  Since we internally represent 
           un-decoded letters with upper case, we also convert everything to 
           uppercase as well. */
        ...
        for (int i = 0; i < ENGLISH_FREQUENCY_ORDER.size(); i++) {
            ...
            double frequency = cryptogramInventory.getLetterPercentage(cipherLetter);
            double previousFrequency = i == 0 ? 
                1 : 
                cryptogramInventory.getLetterPercentage(frequencyOrdering.get(i-1));

            /* If the current frequency is far enough from the previous one, then we 
               assume the letter is in English frequency order.  Go ahead and update 
               the output. */
            if (previousFrequency - frequency > EASY_THRESHOLD) {
    ...
    public void decipherWithDictionary() {
        for (int x = 0; x < NUMBER_DICTIONARY_RUNS; x++) {
            ...
            String[] words = decodedText.replaceAll("[,.?!\"-]", "").split(" ");
            ...
            for (char toReplace : unassignedCipherTextLetters) {
                ...
                for (int word = 0; word < words.length; word++) {
                    /* Only worry about this word if it has the character we're 
                       concerned with. */
                    if (words[word].matches("[a-z-]*(" + toReplace + "+[a-z-]*)+")) {
                        List<Character> workingChoices = new ArrayList<Character>();
                        for (char choice : possibleChoices) {
                            if (dictionary.contains(words[word].replace(toReplace, choice))) {
                                workingChoices.add(choice);
                            }
                        }

                        /* Ignore the case where none of the given possibilities work--
                           this case likely means a word is missing from the dictionary. */
                ...
                /* If we have narrowed down the possibilities for the translation to a 
                single choice, assign that translation, and decode it in the text. */
                if (possibleChoices.size() == 1) {
                    char choice = possibleChoices.get(0);

                    newlyAssigned.add(new Character(Character.toLowerCase(toReplace)));
                    unassignedPlainTextLetters.remove(new Character(choice));

                    decodedText = decodedText.replace(toReplace, choice);
                }
            }
    ...
    /* post: returns a string representation of result of the frequency analysis 
             on the ciphertext; indicates that a letter is untranslated with a '*' */
    public String toString() {
        return decodedText.replaceAll("[A-Z]", "*");
    }
    
    /* post: frequencyOrdering is populated with a list of all lowercase letters ordered 
     *       from most to least frequent in the ciphertext; cryptogramInventory is populated with
     *       a letter inventory corresponding to the ciphertext */
    private void calculateFrequencyOrdering() {
        LetterInventory cryptogramInventory = new LetterInventory(this.cipherText);
        this.cryptogramInventory = cryptogramInventory;

        this.frequencyOrdering = this.getFrequencyOrder();
    
        /* Reverse the frequencyOrdering, because getFrequencyOrder() returns a list
         * from least-to-most frequent, and we want the opposite. */
        Collections.reverse(this.frequencyOrdering);
    }

    /* pre: throws a FileNotFoundException if file does not exist
     * post: cipherText is populated with the contents of file, and 
     * decodedText is initialized to an all uppercase version of the ciphertext */
    ...
    /* post: populates dictionary with the English words listed in the provided dictionary 
             text file */
    private void readDictionary() {
        ...
        try {
            ...
            System.out.println("You must copy dictionary.txt to this directory" +
    ...
    /* post: returns a list of the characters in the ciphertext in frequency order from 
    ...
    private List<Character> getFrequencyOrder() {
        ...
        for (char c = 'a'; c < ('z' + 1); c++) {
            sortedFrequencies.add(this.cryptogramInventory.getLetterPercentage(c));
        }
        ...
        for (int i = 0; i < sortedFrequencies.size(); i++) {
            /* go through all of the possible letters in the inventory, and append 
             * each letter to the order (but only if it's not already in the list) */
            for (char c = 'a'; c < ('z' + 1); c++) {
                if (!order.contains(c) && 
                    Math.abs(this.cryptogramInventory.getLetterPercentage(c) - sortedFrequencies.get(i)) < DELTA) {
                    order.add(c);
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            freq = new FrequencyAnalysis("cryptogram.txt");
        } catch (FileNotFoundException e) {
            System.out.println("You must copy cryptogram.txt to this directory" +
                               " before running the frequency analysis.");
            System.exit(1);
        }
